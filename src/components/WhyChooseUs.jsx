      ...
      <div className="relative w-full max-w-[1440px] mx-auto flex flex-col flex-1 h-full border-x border-white/10">
          ...
          <div className="md:col-span-8 lg:col-span-8 p-6 md:p-8 lg:p-12">
            <div className="max-w-3xl">
              <h2 className="text-3xl md:text-5xl lg:text-[56px] tracking-tight leading-[1.1] mb-6">
                <span className="text-white/80 font-medium">Why choose spectup as </span><span className="text-white font-bold">your<br />capital advisory partner?</span>
              </h2>
        ...
        <div className="p-6 md:p-8 lg:p-12 pb-6">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-auto lg:h-[450px]">
                ...
                {/* Left Block */}
                <div className="border border-white/10 p-8 flex flex-col relative group bg-black hover:bg-[#050505] transition-colors">
                    <div className="absolute top-6 right-6 w-8 h-8 bg-white flex items-center justify-center transition-transform group-hover:-translate-y-1 group-hover:translate-x-1 cursor-pointer z-10 text-black">
                    ...
                    <div className="flex-1 mt-8 mb-4 border-b border-white/10 relative flex items-end gap-[4px] pb-5 min-h-[150px]">
                        {Array.from({ length: 32 }).map((_, i) => {
                            ...
                            return (
                                <div key={i} className="flex-1 bg-white/30 hover:bg-white transition-colors cursor-pointer group-hover:bg-white/50" style={{ height: `${height}%`}}></div>
                            )
                        })}
                ...
                <div className="flex flex-col gap-6 h-[500px] lg:h-auto">
                    <div className="border border-white/10 p-8 flex-1 flex flex-col justify-center relative overflow-hidden group bg-black hover:bg-[#050505] transition-colors">
                        {/* Background arcs */}
                        <div className="absolute top-[-50%] right-[-10%] w-[150%] h-[150%] rounded-full border border-white/20 opacity-40 pointer-events-none group-hover:scale-110 transition-transform duration-700"></div>
                        <div className="absolute top-[-40%] right-[-5%] w-[120%] h-[120%] rounded-full border border-white/20 opacity-40 pointer-events-none group-hover:scale-105 transition-transform duration-700"></div>
                        <div className="absolute top-[-30%] right-[0%] w-[90%] h-[90%] rounded-full border border-white/20 opacity-40 pointer-events-none"></div>

                        <Zap className="absolute top-8 right-8 text-white w-7 h-7 z-10 drop-shadow-[0_0_10px_rgba(255,255,255,0.8)] fill-white" />
                    ...
                    <div className="border border-white/10 p-8 flex-1 flex flex-col justify-center relative overflow-hidden group bg-black hover:bg-[#050505] transition-colors">
                        ...
                        <div className="absolute bottom-[-60%] right-[-20%] w-[180%] h-[180%] rounded-full border border-white/20 opacity-40 pointer-events-none group-hover:rotate-12 transition-transform duration-1000"></div>
                        <div className="absolute bottom-[-40%] right-[-10%] w-[120%] h-[120%] rounded-full border border-white/20 opacity-40 pointer-events-none"></div>
                        <div className="absolute bottom-[-10%] right-[10%] w-[60%] h-[180%] border border-white/20 opacity-40 pointer-events-none rounded-[100%]"></div>
                        <div className="absolute bottom-[20%] right-[-20%] w-[180%] h-[60%] border border-white/20 opacity-40 pointer-events-none rounded-[100%]"></div>
                ...
                {/* Right Block */}
                <div className="border border-white/10 relative overflow-hidden group p-8 flex flex-col justify-end bg-black">
                    <img src={three} alt="Happy Clients" className="absolute inset-0 w-full h-full object-cover opacity-50 group-hover:scale-105 group-hover:opacity-70 transition-all duration-700" />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-black/10"></div>
                    
                    {/* Top left badge */}
                    <div className="absolute top-8 left-8 flex items-center gap-2 z-10">
                        <div className="w-5 h-5 bg-[#C1FF00] flex items-center justify-center pt-[1px] pl-[1px]">
                            <svg className="w-[10px] h-[10px] text-black" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="5"><path d="M4 15l8-8 8 8"/></svg>
                        </div>
                        <span className="text-white font-bold tracking-tight text-sm">spectup</span>
                    </div>
                    
                    {/* Top right rating */}
                    <div className="absolute top-8 right-8 text-right z-10 flex flex-col items-end">
                        <div className="text-[20px] font-bold text-white mb-1 leading-none">4.7 / 5</div>
                    ...
                    <div className="relative z-10">
                        <div className="text-[52px] font-bold tracking-tighter text-white mb-2 leading-none">92%</div>
                        <p className="text-[13px] text-white/80 leading-relaxed max-w-[260px] mb-8 font-medium">
                            Our clients are extremely happy with our capital raising services
                        </p>
                        <div className="flex gap-4 items-center">
                            {/* Trustpilot Logo */}
                            <div className="flex items-center gap-[6px]">
                                <div className="w-[18px] h-[18px] bg-[#00b67a] flex items-center justify-center pt-[2px]">
                                    <span className="text-white text-[14px] leading-none">★</span>
                                </div>
                                <span className="text-white font-bold text-[14px] tracking-wide">Trustpilot</span>
                            </div>
                            {/* Clutch Logo (Text replica) */}
                            <div className="flex items-center gap-[6px]">
                                <span className="text-white font-extrabold text-[15px] tracking-tight">Clutch</span>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div className="px-6 md:px-8 lg:px-12 pb-12 w-full">
            <div className="border border-white/10 flex flex-col md:flex-row items-stretch w-full overflow-hidden bg-black group/marquee">
